...
def apply_custom_css(accent_color="#d32f2f"):
    ...
    <style>
        ...
        /* Active tabs and highlights based om accent color */
        ...
        /* Horizontal layout for Strengths and Weaknesses */
...
def setup_sidebar():
    with st.sidebar:
        ...
        .stButton button, .main-header, .stTabs [aria-selected="true"] {{
            background-color: {theme_color} !imoprtant;
...
def role_selection_section(role_requirements):
    ...
    if not upload_jd:
        ...
        st.markdown(f"<p>Cutoff score for shortlisting: <b>{75}/100</b></p>", unsafe_allow_html=True)
...
def display_analysis_results(analysis_result):
    ...
    with col2:
        if selected:
            ...
            st.markdown("<h2 style='color:#d32f2f;'>👎Unfortunately, you weren't shortlisted.</h2>", unsafe_allow_html=True)
    ...
    with col2:
        report_content = f"""
    # Recruitment Agent - Resume Analysis Report

    ## Overall Score: {overall_score}/100

    Status: {"👍 Shortlisted" if selected else "👎 Not Shortlisted"}
...
def resume_qa_section(has_resume, ask_question_func=None):
    if not has_resume:
        st.warning("Please upload a resume for analysis first")
        return
    ...
    with st.expander("Example Questions"):
        example_questions = [
            ...
            "What education qualifications does the candidate have?",
            "What is the candidate's key achievements?",
            "Has the candidate managed teams before?",
            "What projects has the candidates worked on?",
            "Does the candidate have experience with cloud technology?"
        ]
...
def interview_questions_section(has_resume, generate_questions_func=None):
    if not has_resume:
        st.warning("Please upload a resume for analysis.")
        return
    ...
    if st.button("Generate Interview Questions"):
        if generate_questions_func:
            with st.spinner("Generating personlized interview questions..."):
                questions = generate_questions_func(question_types, difficulty, num_questions)

                download_content = f"# Recruitment Agent - Interview Questions\n\n"
                ...
                if questions:
                    ...
                    href = f'<a class="download-btn" href="data:text/markdown; base64,{b64}" download="airecruitment_interview_questions.md">📝 Download All Questions</a>'
                    st.markdown(href, unsafe_allow_html=True)
...
def resume_improvement_section(has_resume, improve_resume_func=None):
    ...
    if st.button("Generate Resume Improvements"):
        if improve_resume_func:
            with st.spinner("Alanyzing and generating improvements..."):
                improvements = improve_resume_func(improvement_areas, target_role)

                download_content = f"# Recruitment Agent - Resume Improvement Suggestions\n\nTarget Role: {target_role if target_role else 'Not specified'}\n\n"
                ...
                href = f'<a class="download-btn" href="data:text/markdown;base64,{b64}" download="airecruitment_resume_improvements.md">📄 Download All Suggestions</a>'
                st.markdown(href, unsafe_allow_html=True)
        ...
        if get_improved_resume_func:
            with st.spinner("Creating Improved Resume..."):
                ...
                with col1:
                    ...
                    href = f'<a class="download-btn" href="data:file/txt;base64,{b64}" download="airecruitment_improved_resume.txt">📄 Download as TXT</a>'
                    st.markdown(href, unsafe_allow_html=True)
...
"""
                    ...
                    md_href = f'<a class="download-btn" href="data:text/markdown;base64,{md_b64}" download="airecruitment_improved_resume.md">📝 Download as Markdown</a>'
